		...
		<ul class="nav-list">
		  ...
		  <li>
			...
			<a href="com_accs.php">
			  ...
			  <span class="link_name">COMMERCIALISATION DES ACCES</span>
			</a>
			<span class="tooltip">COMMERCIALISATION DES ACCES</span>
		  </li>
		  <li>
			<a href="#">
			  <i class="fas fa-check"></i>
			  <span class="link_name">REALISATIONS BUDGETAIRES					</span>
			</a>
			<span class="tooltip">REALISATIONS BUDGETAIRES	</span>
		  </li>
		  <li>
			<a href="#">
...
<h1> CHIFFRE D'AFFAIRES</h1>
    ...
    <form method="post" action="chiff_aff.php" class="do">
...
if ($annee && $dot) {
    ...
    $sql_re_n_1 = "SELECT mois, $dot FROM  	table_chif_telephone_re_n_1 WHERE annee = '$annee'";
    $result_re_n_1 = $con->query($sql_re_n_1);

    // Requête SQL pour récupérer les données de table_cm_ligne_tel_objectif
    $sql_objectif = "SELECT mois, $dot FROM table_chif_aff_telephone_objectif WHERE annee = '$annee'";
    $result_objectif = $con->query($sql_objectif);

    // Requête SQL pour récupérer les données de table_cm_ligne_tel_realisation
    $sql_realisation = "SELECT mois, $dot FROM  table_chif_telephone_realisation WHERE annee = '$annee'";
...
$total_taux_de_realisation_p = $total_objectif != 0 ? ($total_realisation / $total_objectif) * 100 : 0;
$total_taux_de_realisation_a = $total_objectif != 0 ? ($total_realisation / $total_objectif) * 100 : 0;
...
echo "<table border='1'>";
echo "<tr><th colspan='14'>CHIFFRE D'AFFAIRES/ LIGNE TEL [PSTN]</th></tr>";
echo "<tr>
        <th>MOIS</th>
        <th>REALISE EX [N-1] Par Mois</th>
        <th>REALISE EX [N-1] Période</th>
        <th>OBJECTIF Par Mois</th>
        <th>OBJECTIF Période</th>
        <th>REALISE Par Mois</th>
        <th>REALISE Période</th>
        <th>TAUX(%)REALISE PAR MOIS</th>
        <th>TAUX(%)REALISE PAR PERIODE</th>
        <th>TAUX(%)REALISE PAR ANNEE</th>
        <th>ECART Par Mois</th>
        <th>ECART Période</th>
        <th>RESTE À RÉALISER</th>
        <th>COMPARAISON À L'ANNÉE PRÉCÉDENTE (%)</th>
    </tr>";
...
echo "<td>$total_ecard</td>"; // Pas de valeur pour l'écart annuel
echo "<td>$total_ecard</td>"; // Pas de valeur pour l'écart annuel
echo "<td>$total_rest_a_realiser</td>"; // Pas de valeur pour le reste à réaliser annuel
echo "<td>$comparaison%</td>"; // Pas de valeur pour la comparaison à l'année précédente annuelle
echo "</tr>";
// Fermer la connexion
$con->close();
...
if ($annee && $dot) {
    // Requête SQL pour récupérer les données de table_cm_ligne_tel_re_n_1
    $sql_re_n_1 = "SELECT mois, $dot FROM  	table_chif_intenet_filaire_re_n_1 WHERE annee = '$annee'";
    $result_re_n_1 = $con->query($sql_re_n_1);

    // Requête SQL pour récupérer les données de table_cm_ligne_tel_objectif
    $sql_objectif = "SELECT mois, $dot FROM table_chif_aff_internet_filaire_objectif WHERE annee = '$annee'";
    $result_objectif = $con->query($sql_objectif);

    // Requête SQL pour récupérer les données de table_cm_ligne_tel_realisation
    $sql_realisation = "SELECT mois, $dot FROM  	table_chif_internet_filaire_realisation WHERE annee = '$annee'";
    $result_realisation = $con->query($sql_realisation);
...
echo "<h1>La dot de $dot en $annee :</h1>";
echo "<table border='1'>";
echo "<tr><th colspan='14'>CHIFFRE D'AFFAIRES/ INTERNET [ADSL+FTTH]	
</th></tr>";
echo "<tr>
        <th>MOIS</th>
        <th>REALISE EX [N-1] Par Mois</th>
...
if ($annee && $dot) {
    // Requête SQL pour récupérer les données de table_cm_ligne_tel_re_n_1
    $sql_re_n_1 = "SELECT mois, $dot FROM  	table_chif_intenet_4g_re_n_1 WHERE annee = '$annee'";
    $result_re_n_1 = $con->query($sql_re_n_1);

    // Requête SQL pour récupérer les données de table_cm_ligne_tel_objectif
    $sql_objectif = "SELECT mois, $dot FROM table_chif_aff_internet_4g_objectif WHERE annee = '$annee'";
    $result_objectif = $con->query($sql_objectif);

    // Requête SQL pour récupérer les données de table_cm_ligne_tel_realisation
    $sql_realisation = "SELECT mois, $dot FROM  table_chif_internet_4g_realisation WHERE annee = '$annee'";
    $result_realisation = $con->query($sql_realisation);
...
echo "<h1>La dot de $dot en $annee :</h1>";
echo "<table border='1'>";
echo "<tr><th colspan='14'>CHIFFRE D'AFFAIRES /INTERNET [4G LTE]</th></tr>";
echo "<tr>
        <th>MOIS</th>
        <th>REALISE EX [N-1] Par Mois</th>
        <th>REALISE EX [N-1] Période</th>
